...
#if 0
/*
 * MIPI-CSIS, cameric and cameric-LITE platform devices registration.
 */
static int register_cameric_lite_entity(struct cameric_md *cmd,
				     struct cameric_lite *cameric_lite)
{
	struct v4l2_subdev *sd;
	struct cameric_media_pipeline *ep;
	int ret;

	if (WARN_ON(cameric_lite->index >= cameric_LITE_MAX_DEVS ||
		    cmd->cameric_lite[cameric_lite->index]))
		return -EBUSY;

	sd = &cameric_lite->subdev;
	...
	if (!ret)
		cmd->cameric_lite[cameric_lite->index] = cameric_lite;
	else
		v4l2_err(&cmd->v4l2_dev, "Failed to register cameric.LITE%d\n",
			 cameric_lite->index);
	return ret;
}
#endif
...
#if 0
	for (i = 0; i < cameric_LITE_MAX_DEVS; i++) {
		if (!cmd->cameric_lite[i])
			continue;

		sink = &cmd->cameric_lite[i]->subdev.entity;
		ret = media_create_pad_link(source, pad, sink,
					       FLITE_SD_PAD_SINK, 0);
		if (ret)
			return ret;

		/* Notify cameric-LITE subdev entity */
		ret = media_entity_call(sink, link_setup, &sink->pads[0],
					&source->pads[pad], 0);
		if (ret)
			break;
...
#if 0
/* Create links from cameric-LITE source pads to other entities */
static int __cameric_md_create_flite_source_links(struct cameric_md *cmd)
{
	struct media_entity *source, *sink;
	int i, ret = 0;

	for (i = 0; i < cameric_LITE_MAX_DEVS; i++) {
		struct cameric_lite *cameric = cmd->cameric_lite[i];

		if (cameric == NULL)
			continue;

		source = &cameric->subdev.entity;
		sink = &cameric->ve.vdev.entity;
		/* cameric-LITE's subdev and video node */
		ret = media_create_pad_link(source, FLITE_SD_PAD_SOURCE_DMA,
					       sink, 0, 0);
		if (ret)
			break;
		/* Link from cameric-LITE to IS-ISP subdev */
		sink = &cmd->cameric_is->isp.subdev.entity;
...
#if 0
	ret = __cameric_md_create_flite_source_links(cmd);
	if (ret < 0)
		return ret;
#endif
	if (cmd->use_isp)
