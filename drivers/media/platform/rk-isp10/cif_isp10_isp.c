...
static int cifisp_qbuf(struct file *file, void *priv, struct v4l2_buffer *p)
{
	...
	CIFISP_DPRINT(CIFISP_DEBUG,
		      " %s: %s: p->type %d p->index %d\n",
		      ISP_VDEV_NAME, __func__, p->type, p->index);
	return vb2_core_qbuf(&isp_dev->vb2_vidq, p->index, p);
}
...
static int cifisp_streamon(struct file *file, void *priv, enum v4l2_buf_type i)
{
	struct cif_isp10_isp_dev *isp_dev =
		video_get_drvdata(video_devdata(file));
	int ret = vb2_core_streamon(&isp_dev->vb2_vidq, i);
...
static int cifisp_streamoff(struct file *file, void *priv, enum v4l2_buf_type i)
{
	...
	ret = vb2_core_streamoff(&isp_dev->vb2_vidq, i);
...
static void cif_isp10_vb2_queue(struct vb2_buffer *vb)
{
	...
	spin_lock_irq(&isp_dev->lock);
	list_add_tail(&vb->queued_entry, &isp_dev->stat);
	spin_unlock_irq(&isp_dev->lock);
}

static void cif_isp10_vb2_release(struct vb2_buffer *vb)
{
	struct vb2_queue *vq = vb->vb2_queue;
	struct cif_isp10_isp_dev *isp_dev = vq->drv_priv;

	unsigned long flags;
	spin_lock_irqsave(&isp_dev->lock, flags);
	CIFISP_DPRINT(CIFISP_DEBUG, "Releasing buffer entry!\n");
	list_del_init(&vb->queued_entry);
	spin_unlock_irqrestore(&isp_dev->lock, flags);
	CIFISP_DPRINT(CIFISP_DEBUG, "Releasing buffer exit!\n");	
}

static int cif_isp10_vb2_init(struct vb2_buffer *vb)
{
	return 0;
}
...
{
	struct cif_isp10_isp_dev *isp_dev = vq->drv_priv;
	struct vb2_buffer *buf, *tmp;

	unsigned long flags;
	spin_lock_irqsave(&isp_dev->lock, flags);
	list_for_each_entry_safe(buf, tmp, &isp_dev->stat, queued_entry) {
		list_del_init(&buf->queued_entry);
...
static struct vb2_ops cif_isp10_vb2_ops = {
	...
	.buf_cleanup	= cif_isp10_vb2_release,
	.buf_init	= cif_isp10_vb2_init,
	.wait_prepare	= vb2_ops_wait_prepare,
	.wait_finish	= vb2_ops_wait_finish,
	.stop_streaming	= cif_isp10_vb2_stop_streaming,
};

static int cif_isp10_init_vb2_queue(struct vb2_queue *q, struct cif_isp10_isp_dev *isp_dev)
{
	memset(q, 0, sizeof(*q));
...
{
	int ret;
	struct cif_isp10_v4l2_node *node = vdev_to_node(vdev_cifisp);
	...
	ret = video_register_device(vdev_cifisp, VFL_TYPE_GRABBER, -1);
	if (ret < 0) {
		dev_err(&vdev_cifisp->dev,
			"could not register Video for Linux device\n");
		goto e_init_vdev_ent;
	}
...
e_init_vdev_ent:
	media_entity_cleanup(&vdev_cifisp->entity);
err:
	return ret;
}
	...
	struct cif_isp10_isp_readout_work *meas_work)
{
	...
	stat_buf = (struct cifisp_stat_buffer *)vb->vb2_queue->mem_ops->vaddr(vb->planes[0].mem_priv);
