	...
	enum cif_isp10_img_src_state state)
{
	int ret = 0;
	//TODO(zsq)
	struct v4l2_subdev *sensor = dev->subdevs[CIF_ISP10_SD_SENSOR];
	struct v4l2_subdev *flash  = dev->subdevs[CIF_ISP10_SD_FLASH];
	...
	switch (state) {
	case CIF_ISP10_IMG_SRC_STATE_OFF:
		// TODO(zsq) flash subdev is not ready yet.
		if (flash)
			v4l2_info(flash, "TODO: set flash sd mode\n");
		ret = v4l2_subdev_call(sensor, core, s_power, 0);
		break;
	case CIF_ISP10_IMG_SRC_STATE_SW_STNDBY:
		if (dev->img_src_state == CIF_ISP10_IMG_SRC_STATE_STREAMING) {
			ret = v4l2_subdev_call(sensor, video, s_stream, 0);
		} else {
...
static int cif_isp10_subdev_enum_strm_fmts(
...
int cif_isp10_subdev_s_strm_fmt(struct v4l2_subdev *subdev,
	...
	struct cif_isp10_device *dev)
{
	...
	for (index = 0;; index++) {
		if (IS_ERR_VALUE(cif_isp10_subdev_enum_strm_fmts(sensor,
			index, &strm_fmt_desc)))
			break;
		if (!strm_fmt_desc.discrete_frmsize) {
			img_src_width = clamp(target_width,
					      strm_fmt_desc.min_frmsize.width,
					      strm_fmt_desc.max_frmsize.width);
			img_src_height = clamp(target_height,
		...
		if ((img_src_width >= target_width) &&
			...
			u32 diff = abs(target_height - (target_width * img_src_height / img_src_width));
			if (matching_format_found) {
				int strm_fps = strm_fmt_desc.min_intrvl.denominator / strm_fmt_desc.min_intrvl.numerator;
				int req_fps = request_strm_fmt.frm_intrvl.denominator / request_strm_fmt.frm_intrvl.numerator;
				if (dev->config.jpeg_config.enable &&
				    img_src_width >= request_strm_fmt.frm_fmt.width &&
				    img_src_height > request_strm_fmt.frm_fmt.height)
					/*
					 * for image capturing we try to
					 * maximize the size
					 */
					better_match = true;
				else if (!dev->config.jpeg_config.enable && strm_fps > req_fps)
					/* maximize fps */
					better_match = true;
				else if (!dev->config.jpeg_config.enable && strm_fps == req_fps && diff < best_diff)
					/*
	...
	ret = cif_isp10_subdev_s_strm_fmt(sensor, &request_strm_fmt);
	if (IS_ERR_VALUE(ret))
		goto err;
...
static int cif_isp10_config_img_src(struct cif_isp10_device *dev)
{
	int ret = 0;
	//TODO(zsq)
	//struct v4l2_subdev *sensor = dev->subdevs[CIF_ISP10_SD_SENSOR];
	...
	if (!dev->sp_stream.updt_cfg && !dev->mp_stream.updt_cfg)
		return 0;

	/* TODO(zsq) Make it right
	...
	if (IS_ERR_VALUE(ret))
		goto err;
	*/
	dev->config.cam_itf.type = PLTFRM_CAM_ITF_MIPI;
	dev->config.cam_itf.cfg.mipi.dphy_index = 0;
	dev->config.cam_itf.cfg.mipi.vc = 0;
	dev->config.cam_itf.cfg.mipi.nb_lanes = 2;
	dev->config.cam_itf.cfg.mipi.bit_rate = 840;

	//TODO(zsq) SENSOR_MODE_DATA is deleted
	cif_isp10_pltfrm_pr_dbg(dev->dev,
	...
	struct cif_isp10_device *dev)
{
	...
	/* mipi_dphy */
	sd_dphy = dev->subdevs[CIF_ISP10_SD_PHY_CSI];
	BUG_ON(!sd_dphy);
	v4l2_subdev_call(sd_dphy, core, s_power, 1);
	v4l2_subdev_call(sd_dphy, video, s_stream, 1);

	cif_iowrite32(mipi_ctrl | 0xf << 8, //TODO(zsq): for phy0/phy1, shutdown lanes is different?
		dev->config.base_addr + CIF_MIPI_CTRL);
	...
	struct cif_isp10_device *dev)
{
	...
	if (mi_mis_tmp & CIF_MI_DMA_READY)
		cif_isp10_pltfrm_pr_err(dev->dev,
					"dma icr err: 0x%x\n",
					mi_mis_tmp);
	vb2_buffer_done(&dev->dma_stream.curr_buf->vb.vb2_buf, VB2_BUF_STATE_DONE);
	...
	enum cif_isp10_stream_id stream_id)
{
			...
			if (jpe_status & CIF_JPE_STATUS_ENCODE_DONE) {
				cif_iowrite32(CIF_JPE_STATUS_ENCODE_DONE,
					dev->config.base_addr +
						CIF_JPE_STATUS_ICR);
				if (stream->curr_buf) {
					struct vb2_buffer *curr_buf = &stream->curr_buf->vb.vb2_buf;
					curr_buf->planes[0].length=
					cif_ioread32(dev->config.base_addr +
						CIF_MI_BYTE_CNT);
					cif_isp10_pltfrm_pr_dbg(NULL,
						"JPEG encoding done, size %u\n",
						curr_buf->planes[0].length);
					if (cif_ioread32(dev->config.base_addr +
						CIF_MI_RIS) & CIF_MI_WRAP_MP_Y)
						cif_isp10_pltfrm_pr_err(NULL,
							"buffer wrap around detected, JPEG presumably corrupted (%d/%d/%u)\n",
	...
	if ((!stream->next_buf) &&
		...
		(vb2_dma_contig_plane_dma_addr(&stream->next_buf->vb.vb2_buf, 0) !=
			cif_ioread32(y_base_addr))) {
		cif_isp10_pltfrm_pr_warn(dev->dev,
		...
		if (stream->curr_buf && stream->next_buf) {
			...
			if (stream->metadata.d && dev->isp_dev.streamon) {
				...
				if (work) {
					...
					work->vb = &stream->curr_buf->vb.vb2_buf;
			...
			if (wake_now) {
				cif_isp10_pltfrm_pr_dbg(NULL,
					"frame done\n");
				wake_up(&stream->curr_buf->vb.vb2_buf.vb2_queue->done_wq);
			}
			stream->curr_buf = NULL;
		}
 ...
 * mi will stop;
 */
			*next_buff_addr = vb2_dma_contig_plane_dma_addr(&stream->curr_buf->vb.vb2_buf, 0);
		}
	}
	(void)update_mi(dev);
	...
	cif_isp10_pltfrm_pr_dbg(dev->dev,
		...
		(stream->curr_buf) ? (int)vb2_dma_contig_plane_dma_addr
		(&stream->curr_buf->vb.vb2_buf, 0) : -1,
	...
	u32 stream_ids)
{
	...
	/*TODO(zsq) revise src_exps valid frms
	...
	u32 stream_ids)
{
	...
	if ((dev->sp_stream.state == CIF_ISP10_STATE_DISABLED) &&
		...
		//TODO(zsq): read from dts
		dev->config.input_sel = CIF_ISP10_INP_CSI;
	...
	dev->config.isp_config.input = &dev->config.img_src_output.frm_fmt;
	return 0;
err:
	cif_isp10_pltfrm_pr_err(NULL,
		"failed with error %d\n", ret);
	...
	int stream_ids)
{
	...
	if ((dev->sp_stream.state == CIF_ISP10_STATE_DISABLED) &&
		(dev->mp_stream.state == CIF_ISP10_STATE_DISABLED)) {
		struct v4l2_subdev *phy;

		phy = dev->subdevs[CIF_ISP10_SD_PHY_CSI];
		v4l2_subdev_call(phy, video, s_stream, 0);
		v4l2_subdev_call(phy, core, s_power, 0);
...
int register_cifisp_isp_subdev(struct cif_isp10_device *isp_dev,
	struct v4l2_device *v4l2_dev)
{
	struct v4l2_subdev *sd = &isp_dev->isp_sub_dev;
	struct v4l2_ctrl_handler *handler = &isp_dev->ctrl_handler;
	int ret;
	...
	isp_dev->pads[CIF_ISP10_ISP_PAD_SINK_DMA].flags = MEDIA_PAD_FL_SINK;
...
void unregister_cifisp_isp_subdev(struct cif_isp10_device *isp_dev)
{
	struct v4l2_subdev *sd = &isp_dev->isp_sub_dev;
			     ...
			     enum cif_isp10_stream_id stream_id))
{
	...
	dev->vs_wq = NULL; //TODO(zsq) revise src_exps valid frms
	...
	unsigned int input_index)
{
	...
	return dev_driver_string(dev->subdevs[CIF_ISP10_SD_SENSOR]->dev);
}
	...
	u32 *size)
{
	...
	if (stream->state < CIF_ISP10_STATE_READY) {
		cif_isp10_pltfrm_pr_info(NULL,
			"%s stream not ready, return max size\n",
			cif_isp10_stream_id_string(stream_id));
		*size = 4032 * 3024 * 2; //TODO(zsq)
		return 0;
	}
	...
	int val)
{
	...
	case CIF_ISP10_CID_JPEG_QUALITY:
		...
		break;
	case CIF_ISP10_CID_FLASH_MODE:
		//TODO(zsq) set flash mode via flash subdev
		break;
	case CIF_ISP10_CID_WB_TEMPERATURE:
	case CIF_ISP10_CID_ANALOG_GAIN:
	case CIF_ISP10_CID_EXPOSURE_TIME:
